...
export const directAnswers: DirectAnswer[] = [
  ...
  {
    query: "antar alkohol Ubud",
    path: "/id/blog/alcohol-delivery-ubud-tanah-lot",
    answer:
      "MadeBrings antar ke arah Ubud dari Abianbase jika pin villa dan jalan hari itu masuk akal. Chat dulu. Katalog sama, 21+, tunai atau transfer.",
  },
  {
    ...
    ...commercial.flatMap((post) => [
      ...
      post.idCopy
        ? [
            `#### ${post.idCopy.title}`,
            "",
            post.idCopy.speakable ?? post.idCopy.excerpt,
            "",
            `Source: ${origin}/id/blog/${post.slug}`,
            "",
          ].join("\n")
        : "",
    ]),
